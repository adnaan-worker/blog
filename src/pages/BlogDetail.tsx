...
import { FiCalendar, FiClock, FiTag, FiUser, FiShare2, FiMessageCircle, FiHeart, FiBookmark, FiArrowLeft } from 'react-icons/fi';
import { 
  PageContainer,
...
const BlogDetail: React.FC = () => {
  ...
  useEffect(() => {
    const fetchArticle = async () => {
      ...
      if (foundArticle) {
        // 基于相同分类或标签查找相关文章
        const related = DUMMY_ARTICLES.filter(a => 
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
          ...
          <div style={{ display: 'flex', gap: '0.5rem' }}>
            ...
            <ArticleActionButton onClick={handleBookmark}>
              <FiBookmark size={16} color={bookmarked ? 'var(--accent-color)' : undefined} /> {bookmarked ? '已收藏' : '收藏'}
            </ArticleActionButton>
          ...
          <BlogSidebar style={{ alignSelf: 'flex-start', position: 'sticky', top: '80px' }}>
            ...
            <SidebarCard>
              ...
              <CategoryList>
                ...
                <CategoryItem onClick={() => document.querySelectorAll('h2')[1]?.scrollIntoView({ behavior: 'smooth' })}>
                  <span>组件设计的最佳实践</span>
                </CategoryItem>
                <CategoryItem onClick={() => document.querySelectorAll('h2')[2]?.scrollIntoView({ behavior: 'smooth' })}>
                  <span>状态管理优化</span>
                </CategoryItem>
                <CategoryItem onClick={() => document.querySelectorAll('h2')[3]?.scrollIntoView({ behavior: 'smooth' })}>
                  <span>性能调优技巧</span>
                </CategoryItem>
                <CategoryItem onClick={() => document.querySelectorAll('h2')[4]?.scrollIntoView({ behavior: 'smooth' })}>
                  <span>常见陷阱的规避</span>
                </CategoryItem>
              </CategoryList>
            </SidebarCard>
